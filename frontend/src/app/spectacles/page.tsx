...
export default function Spectacles() {
    ...
    <div className="text-white bg-black">
      ...
      <section ref={sectionRef}  className="relative min-h-[60vh] md:min-h-screen flex flex-col justify-center items-center text-center px-4 md:px-6 pt-24 md:pt-24">
        ...
        <div className="relative z-10 max-w-6xl text-white">
          <h1 className=  {`text-3xl md:text-6xl font-bold mt-20 md:mt-18 mb-18 md:mb-24 ${isSectionVisible ? "animate-slideInFromTop" : "opacity-0"
              }`}>
            Spectacles</h1>
          <p className=
  ...
  ${isSectionVisible ? "animate-slideInFromBottom delay-200" : "opacity-0"}`}>
          ...
          <button onClick={handleScrollToRecents} className={`mt-8 md:mt-12 focus:outline-none transition-all duration-1000 ease-in-out 
  ${isSectionVisible ? "animate-slideInFromBottom delay-300" : "opacity-0"}`}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 md:h-10 md:w-10 text-white mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          >
            {spectacles.map((spectacle, index) => (
              <div key={index} className="relative bg-gray-900 rounded-lg overflow-hidden shadow-lg transition transform hover:scale-105">
                ...
                {/* Description */}
                <div className="p-4">
                  <p className="text-sm sm:text-base">{spectacle.description}</p>
          ...
          >
            {galleryImages.map((src, index) => (
              <button key={index} onClick={() => setSelectedImage(src)} className="focus:outline-none">
                <Image src={src} alt={`Galerie ${index + 1}`} width={300} height={300}  className="rounded-lg object-cover w-full h-40 sm:h-48 md:h-60 transition-transform hover:scale-105"/>
              </button>
            ))}
          </div>
        </section>
