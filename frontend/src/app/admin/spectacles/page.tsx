...
export default function SpectaclesPage() {
  ...
  const [editingItem, setEditingItem] = useState<Spectacle | null>(null);
  ...
  const loadData = async () => {
    try {
      const data = await adminApi.getContent<Spectacle[]>('spectacles');
      setSpectacles(data);
    } catch (err) {
  ...
  const handleAdd = () => {
    setEditingItem({ title: '', image: '', description: '', link: '' });
    setEditingIndex(-1);
  };

  const handleEdit = (item: Spectacle, index: number) => {
    setEditingItem({ ...item });
  ...
  const handleDelete = async () => {
    ...
    try {
      await adminApi.deleteItem('spectacles', deleteIndex);
      await loadData();
      setDeleteIndex(null);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingIndex === -1) {
        await adminApi.addItem('spectacles', editingItem);
      } else if (editingIndex !== null) {
        await adminApi.updateItem('spectacles', editingIndex, editingItem);
      }
      await loadData();
      setEditingItem(null);
    ...
    <AdminLayout title="Spectacles">
      ...
      {editingItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-lg bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-semibold">
                {editingIndex === -1 ? 'Ajouter un spectacle' : 'Modifier le spectacle'}
              </h3>
            ...
            <FormField
              ...
              placeholder="/spectacles/nom-du-spectacle"
            />
      ...
      <ConfirmDialog
        ...
        message="Êtes-vous sûr de vouloir supprimer ce spectacle ? Cette action est irréversible."
