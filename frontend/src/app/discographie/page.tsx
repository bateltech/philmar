...
export default function Discographie() {
  ...
  // Use the intersection observer hook for the title
  const [titleRef, isTitleVisible] = useIntersectionObserver({ threshold: 0.1 });

  // Use the intersection observer hook for the filter buttons and album grid
  ...
  return (
    <div className="min-h-screen bg-[length:100%_auto] bg-top bg-no-repeat" style={{ backgroundImage: "url('/images/sun_bg.png')" }}>
      <div className="container mx-auto py-12 md:py-24 px-4">
        ...
        <h1
          ref={titleRef}
          className={`text-4xl font-bold text-white mt-[8rem] mb-12 md:mb-24 ml-4 md:ml-12 ${isTitleVisible ? "animate-slideInFromLeft" : "opacity-0"
            }`}
        >
          Discographie
        </h1>

        {/* Filter Buttons and Album Grid with Slide-In from Bottom Animation */}
        <div
          ref={contentRef}
          className={`${isContentVisible ? "animate-slideInFromBottom" : "opacity-0"
            }`}
        >
          {/* Filter Buttons */}
          <div className="flex flex-col md:flex-row justify-between mx-4 md:mx-8 lg:mx-16">
            <div className="flex flex-wrap  gap-4 mb-8">
              <button
                onClick={() => setSelectedGenre('all')}
                className={`px-4 md:px-6 py-2 ${selectedGenre === 'all' ? 'bg-blue-950 border border-blue-950' : 'border border-white'} text-white cursor-pointer hover:bg-blue-950 border hover:border-blue-950`}
              >
                Tout
              </button>
              <button
                onClick={() => setSelectedGenre('album')}
                className={`px-4 md:px-6 py-2 ${selectedGenre === 'album' ? 'bg-blue-950 border border-blue-950' : 'border border-white'} text-white cursor-pointer hover:bg-blue-950 border hover:border-blue-950`}
              >
                Albums
              </button>
              <button
                onClick={() => setSelectedGenre('enregistrement')}
                className={`px-4 md:px-6 py-2 ${selectedGenre === 'enregistrement' ? 'bg-blue-950 border border-blue-950' : 'border border-white'} text-white cursor-pointer hover:bg-blue-950 border hover:border-blue-950`}
              >
                Enregistrements
              </button>
              <button
                onClick={() => setSelectedGenre('conte')}
                className={`px-4 md:px-6 py-2 ${selectedGenre === 'conte' ? 'bg-blue-950 border border-blue-950' : 'border border-white'} text-white cursor-pointer hover:bg-blue-950 border hover:border-blue-950`}
              >
                Contes
              </button>
            </div>
